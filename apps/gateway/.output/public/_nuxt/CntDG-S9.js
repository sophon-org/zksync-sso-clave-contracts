...
class R extends p {
  constructor({ result: r, url: e }) {
    super(
      ...
      {
        metaMessages: [`Gateway URL: ${m(e)}`, `Response: ${y(r)}`],
...
async function C(n, { blockNumber: r, blockTag: e, data: o, to: c }) {
  const { args: d } = k({ data: o, abi: [S] }),
    [a, i, f, t, s] = d,
    { ccipRead: u } = n,
    g =
      u && typeof (u == null ? void 0 : u.request) == "function"
        ? u.request
        : q;
  try {
    if (!b(c, a)) throw new $({ sender: a, to: c });
    const l = await g({ data: f, sender: a, urls: i }),
      { data: w } = await E(n, {
        blockNumber: r,
        blockTag: e,
        data: L([t, O([{ type: "bytes" }, { type: "bytes" }], [l, s])]),
        to: c,
      });
    return w;
  } catch (l) {
...
async function q({ data: n, sender: r, urls: e }) {
  ...
  for (let d = 0; d < e.length; d++) {
    const a = e[d],
      ...
      f = i === "POST" ? { data: n, sender: r } : void 0;
    try {
      ...
      let s;
      if (
        ((c = t.headers.get("Content-Type")) != null &&
      ...
      if (!x(s)) {
        o = new R({ result: s, url: a });
        continue;
      }
      return s;
    } catch (t) {
      o = new h({ body: f, details: t.message, url: a });
    }
  }
  throw o;
}
