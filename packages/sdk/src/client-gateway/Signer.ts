...
import { createZksyncWalletClient } from "../client/index";
import { StorageItem } from "../utils/storage";

import type { Address, Chain, Hash, Transport } from "viem";
import type { ZksyncAccountWalletClient } from "../client/index";
...
export class Signer implements SignerInterface {
  ...
  private _chainsInfo = new StorageItem<ChainsInfo>(
    StorageItem.scopedStorageKey("chainsInfo"),
    []
  );
  ...
  constructor({
    ...
    this._account = new StorageItem<Account | null>(
      StorageItem.scopedStorageKey("account"),
      null,
      {
        onChange: (newValue) => {
          if (newValue) {
  ...
  async handshake(): Promise<Address[]> {
    ...
    if (this.sessionParameters) {
      try {
        ...
        console.error(
          "Failed to get session data. Proceeding connection with no session.",
          error
        );
      }
    }
  ...
  switchChain(chainId: number): boolean {
    ...
    if (!chainInfo) {
      console.error(
        `Chain ${chainId} is not supported or chain info was not set during handshake`
      );
      return false;
    }
    if (!chain) {
  ...
  private async tryLocalHandling<T>(
    ...
    switch (request.method as Method) {
      ...
      case "wallet_switchEthereumChain": {
        ...
        const switched = this.switchChain(
          typeof chainId === "string" ? hexToNumber(chainId as Hash) : chainId
        );
  ...
  private async sendRpcRequest<T>(
    request: RequestArguments
  ): Promise<RPCResponseMessageSuccessful<T>> {
    ...
    const response: RPCResponseMessage<T> =
