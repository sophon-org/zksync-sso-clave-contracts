...
contract ExampleAuthServerPaymaster is IPaymaster, Ownable {
  ...
  bytes4 public immutable DEPLOY_ACCOUNT_SELECTOR;
  bytes4 public immutable CREATE_SESSION_SELECTOR;
  ...
  constructor(
    ...
    bytes4 deployAccountSelector,
    address sessionKeyValidatorAddress,
    bytes4 createSessionSelector
  ) {
    AA_FACTORY_CONTRACT_ADDRESS = aaFactoryAddress;
    DEPLOY_ACCOUNT_SELECTOR = deployAccountSelector;
    SESSION_KEY_VALIDATOR_CONTRACT_ADDRESS = sessionKeyValidatorAddress;
    CREATE_SESSION_SELECTOR = createSessionSelector;
  }
  ...
  function validateAndPayForPaymasterTransaction(
    ...
    require(
      address(uint160(_transaction.to)) == AA_FACTORY_CONTRACT_ADDRESS ||
        address(uint160(_transaction.to)) == SESSION_KEY_VALIDATOR_CONTRACT_ADDRESS,
    ...
    if (address(uint160(_transaction.to)) == AA_FACTORY_CONTRACT_ADDRESS) {
      require(methodSelector == DEPLOY_ACCOUNT_SELECTOR, "Unsupported method");
    }
    if (address(uint160(_transaction.to)) == SESSION_KEY_VALIDATOR_CONTRACT_ADDRESS) {
