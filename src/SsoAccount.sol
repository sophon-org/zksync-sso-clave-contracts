  ...
  BootloaderAuth
{
  ...
  /// @param _nonce The nonce value for the account to be stored in the Nonce Holder.
  function _incrementNonce(uint256 _nonce) private {
    uint192 nonceKey = uint192(_nonce >> 64);
    bytes memory nonceCall;
    if (nonceKey == 0) {
